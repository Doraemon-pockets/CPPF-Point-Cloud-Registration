...
class RandomTransformSE3:
    ...
    def apply_transform(self, p0, transform_mat):
        ...
        if p0.shape[1] == 7:  # Need to rotate normals also   #修改为7
            n1 = so3.transform(transform_mat[:3, :3], p0[:, 3:6])
            # p1 = np.concatenate((p1, n1), axis=-1)
            p1 = np.concatenate((p1, n1, p0[:, 6:7]), axis=-1)   #将最后一维曲率进行添加返回
...
class ShufflePoints:
    ...
    def __call__(self, sample):
        if 'points' in sample:
            ...
            # sample['points_ref'] = np.loadtxt(r'E:\0\centroid_StitchingRes0246 - Cloud.txt').astype(np.float32)
            sample['points_ref'] = np.loadtxt(r'E:\make_data\ture\centroid_scan_0010_0 - Cloud.txt').astype(np.float32)
            sample['points_src'] = np.random.permutation(sample['points_src'])
            # sample['points_src'] = np.loadtxt(r'G:\pointcloud\Registration\make_data\data\scan\data4096\class\up\scan_0045_11.txt').astype(np.float32)
            # sample['points_src'] = np.loadtxt(r'G:\pointcloud\Registration\make_data\data\scan\data4096\class\up\scan_0045_11.txt').astype(np.float32)
        return sample

...
def sinkhorn(log_alpha, n_iters: int = 5, slack: bool = True, eps: float = -1) -> torch.Tensor:
    ...
    Args:
        ...
        eps: eps for early termination (Used only for handcrafted CPPF). Set to negative to disable.
...
class CPPFNet(nn.Module):
    ...
    def forward(self, data, num_iter: int = 1):
        ...
        xyz_ref, norm_ref, cur_ref = data['points_ref'][:, :, :3], data['points_ref'][:, :, 3:6], data['points_ref'][:, :, 6:7]
        xyz_src, norm_src, cur_src = data['points_src'][:, :, :3], data['points_src'][:, :, 3:6], data['points_src'][:, :, 6:7]
        xyz_src_t, norm_src_t, cur_src_t  = xyz_src, norm_src, cur_src
        ...
        for i in range(num_iter):
            ...
            feat_src = self.feat_extractor(xyz_src_t, norm_src_t, cur_src_t)
            feat_ref = self.feat_extractor(xyz_ref, norm_ref, cur_ref)
...
class CPPFNetEarlyFusion(CPPFNet):
    """Early fusion implementation of CPPFNet, as described in the paper"""
    def __init__(self, args: argparse.Namespace):
        super().__init__(args)
...
def get_model(args: argparse.Namespace) -> CPPFNet:
    return CPPFNetEarlyFusion(args)

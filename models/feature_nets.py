...
_raw_features_sizes = {'xyz': 3, 'dxyz': 3, 'ppf': 6}
...
class FeatExtractionEarlyFusion(nn.Module):
    ...
    def __init__(self, features, feature_dim, radius, num_neighbors):
        ...
        self.cross_attention = CrossAttention(feature_dim * 2, feature_dim)
        self.postpool = get_postpool(feature_dim, feature_dim)

    def forward(self, xyz, normals, curvature):
        ...
        """
        features = sample_and_group_multi(-1, self.radius, self.n_sample, xyz, normals, curvature)  #curvature dimension
        ...
        cross_attention_feat = self.cross_attention(new_feat)

        pooled_feat = torch.max(cross_attention_feat, 2)[0]  # Max pooling (B, C, N)

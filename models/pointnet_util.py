...
#该函数假设输入的点集已经进行了L2归一化，即假设点集中的每个点都被归一化为单位长度向量。函数计算源点集和目标点集之间的夹角距离，用于衡量它们之间的相似性或差异性。
...
#该函数用于计算源点集和目标点集中每两个点之间的欧氏距离的平方，用于衡量它们之间的距离或相似性。
...
#该函数用于根据索引从输入点集中检索相关的点，以便在后续的处理中使用这些点进行计算或分析。
...
def sample_and_group_multi(npoint: int, radius: float, nsample: int, xyz: torch.Tensor, normals: torch.Tensor,
                           curvature: torch.Tensor ,returnfps: bool = False) -> object:     #增加曲率维度
    ...
    if npoint > 0:
        ...
        new_xyz = xyz
        nr = normals[:, :, None, :]
        cr = curvature.repeat(1, 1, nsample)  # 将最后一维扩展为 (1, 2048, 64)
    ...
    ppf_feat = torch.stack([nr_d, ni_d, nr_ni, d_norm, cr, ci], dim=-1)  # (B, npoint, n_sample, 6)
